...
app.post('/api/admin/users/:id/ban', adminOnly, (req, res) => {
  ...
  user.accountStatus = req.body.status || 'banned'; // 'active', 'banned', 'suspended'
...
app.post('/api/webhooks/payment', (req, res) => {
  // Mock WeChat/Alipay payment gateway callback
  const { out_trade_no, total_fee, result_code } = req.body;
  if (result_code === 'SUCCESS') {
    // Imagine out_trade_no encodes userId or maps to a DB order
    const userId = 'user_1'; // Hardcoded for mock
    const user = users.find(u => u.id === userId);
    if (user) user.balance += (total_fee / 100); // 1 RMB = 1 Balance
    console.log(`Payment received for ${out_trade_no}, balance updated`);
  }
  res.send('SUCCESS'); // Return plain text for WeChat API
});
app.post('/api/webhooks/livekit', (req, res) => {
  // Mock LiveKit webhook (participant joined/left)
  const event = req.body.event; // e.g. 'participant_joined', 'participant_left'
...
app.get('/api/search', (req, res) => {
  ...
  const matchedScripts = scripts.filter(s => s.title.toLowerCase().includes(q) || s.tags.some(t => t.toLowerCase().includes(q)));
...
app.post('/api/auth/register', (req, res) => {
  ...
  const newUser = {
    ...
    wishlist: [], checkinStreak: 0, lastCheckin: 0, isAdmin: false, accountStatus: 'active'
  };
  users.push(newUser);
...
app.get('/api/scripts', (req, res) => {
  let result = scripts.filter((s:any) => s.approvalStatus === 'approved');
...
app.post('/api/scripts', (req, res) => {
  ...
  const newScript = {
    ...
    rating: 0, roles: roles || [], isUgc: true, authorId, acts: {}, approvalStatus: 'pending'
  };
